...
| **Turn-Memory** | **Sync** | 턴-초기화, 턴-전체-메모라이즈, 턴-스펙반영 | 프로젝트 지식의 항상성 및 정합성 유지 |
| **Turn-Order** | **Execution** | 턴-태스크-오더/프로그레스/리포트, 턴-태스크-메모라이즈 | 실질적인 프로젝트 전진 및 기여 |
...
- **턴-초기화 (Cleanup)**: 활성 작업 공간(`active/`)에서 완료된 오더와 로그를 `history/`로 이동.
- **턴-전체-메모라이즈 (Knowledge Sync)**: 최신 히스토리와 스펙을 전수조사하여 원자적 **Memory Cell**로 응축.
- **턴-스펙반영 (Spec Update)**: 업데이트된 메모리를 바탕으로 `docs/specs/` 문서를 역동기화.

## 3. ⚡ Turn-Order (작업 실행 체계)
실제 기능을 개발하거나 문제를 해결하는 단계.

- **턴-태스크-오더/프로그레스/리포트 (Execution Cycle)**: Planning, Execution, Reporting 과정을 통한 실제 작업 수행.
- **턴-태스크-메모라이즈 (Micro-Sync)**: 작업 중 습득한 파편화된 정보를 관련 메모리 셀 및 스펙에 즉시 반영.
