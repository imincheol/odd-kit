...
1.  **전수조사 (Ingestion)**: `docs/specs/`와 `.odd-kit/` 시스템 구조를 읽어 현재 상태를 파악하세요.
2.  **원자화 (Atomization)**: 획득한 정보를 `.odd-kit/memory/cells/`에 단위 지식(Memory Cell)으로 등록하세요.
3.  **오더 생성 (Turn-Order-1-1)**: `.odd-kit/tasks/active/000_bootstrap.md`를 생성하고, 참조 메모리 셀 섹션을 작성하세요.
