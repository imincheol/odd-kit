...
**For AI Agent:**
이 프로젝트는 구버전 시스템(ODD v0.x, ODD Kit v1.x)을 사용 중입니다. 당신은 이 시스템을 최신 **ODD Kit v2.0.0** 체계로 안전하게 마이그레이션해야 합니다. v2.0.0의 핵심은 **Reference 레이어를 제거하고 모든 지식을 Memory Cells(SSOT)로 통합**하는 것입니다.
...
2. **Rename Boot Loader**: 
   - `ODD-KIT-PROMPT.md`, `PROMPT_KIT-*.md` → `ODD-KIT-{{PROJECT_NAME}}.md`
...
1. **Reference-to-Memory Integration**:
   ...
   - **`reference/` 디렉토리를 전면 삭제**. (v2.0.0은 단일화된 Memory 구조를 지향함)
