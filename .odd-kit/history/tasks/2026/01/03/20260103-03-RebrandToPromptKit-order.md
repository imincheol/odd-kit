...
title: "Rebrand System to ODD Kit"
status: "in_progress"
priority: "high"
assignee: "odd-agent"
created-at: "2026-01-03T17:35:00"
references: 
  - "ODD-KIT-PROMPT.md"
  - "README.md"
  - "AI-FRAMEWORKS.md"
...
# 📋 Order: 시스템 명칭을 'ODD Kit'으로 변경 및 정체성 재정립

## 1. 배경 (Background)
기존 'ODD (Old Order Driven Development)' 명칭이 Google 'Spec Kit', Claude 'Skills'와 비교했을 때 직관적이지 않다는 피드백이 있음.
시스템의 핵심 가치인 "오더 수행 시 참조를 통해 최적의 프롬프트를 구성하는 구조"를 명확히 드러내기 위해 **'ODD Kit'**으로 리브랜딩이 필요함.

## 2. 목표 (Objectives)
- **리브랜딩**: 시스템 이름을 `ODD`에서 `ODD Kit`으로 전면 변경.
- **정체성 확립**: `Spec Kit`(설계), `Claude Skills`(능력)와의 명확한 포지셔닝(맥락/프롬프트 최적화) 정립.
- **문서화 강화**: 비교 문서를 루트 디렉토리로 이동하여 접근성 강화.

## 3. 작업 계획 (Tasks)
- [x] **비교 문서 격상**: `oddtemplate/.../COMPARISON_AI-FRAMEWORKS.md`를 루트의 `AI-FRAMEWORKS.md`로 이동 및 내용 업데이트.
- [x] **README.md 업데이트**: 메인 타이틀 및 설명 수정, 비교 가이드 섹션 강화.
- [x] **Entry Point 수정**: `ODD-KIT-PROMPT.md` 내 시스템 명칭 변경.
