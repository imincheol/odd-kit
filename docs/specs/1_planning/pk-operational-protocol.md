# 📖 Specification: Prompt Kit Operational Protocol (v1.3.0)

> **Status**: Verified by Memory Cell `pk-operational-protocol-v130.md`

## 1. 개요
본 문서는 Prompt Kit 시스템의 운영 프로세스를 규정한다. 본 시스템은 지식의 **원자화(Atomization)**와 **동기화(Synchronization)**를 통해 AI의 문맥 유지 능력을 극대화한다.
...
- 에이전트는 `.prompt-kit/` 내의 주요 설정과 로드맵을 읽고 현재 작업의 맥락을 완벽히 파악한다.
...
v1.3.0 도입 과정에서 발생한 프로토콜 위반 사례와 그 복구 논리를 기록합니다.
...
- **Physical Trace**: 실습 사례와 실수의 원인을 별도의 메모리 셀(`pk-v130-evolution-log.md`)로 박제하여 지식화한다.
...
---
**Verified by Memory Cell**: `pk-operational-protocol-v130.md`, `pk-v130-evolution-log.md`
...
5. **Accumulative Integration (누적적 통합)**: README 등 외부 노출 문서는 기존의 가치 있는 콘텐츠를 보존(Restoration)하고 새로운 내용을 추가(Addition)하여 통합한다.
