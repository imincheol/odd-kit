...
## 1. Overview
`odd-kit-prompt-template/setup/install.sh`는 단순한 복사 스크립트가 아니라, **Self-Updating Bootstrapper**이자 **Context Injector**입니다. 사용자가 구버전 스크립트를 실행하더라도 항상 최신 로직을 보장하며, 프로젝트 고유의 정체성을 부여합니다.
...
- **Placeholder Replacement**: `odd-kit-prompt-template/`의 모든 문서를 복제할 때, 내부의 `{{PROJECT_NAME}}`, `{{ODD-KIT_PROMPT_NAME}}` 등의 변수를 실제 값으로 치환하여 문맥적 무결성을 확보합니다.
